...
// ═══ 0808 補齊甘特圖:把設計案時程匯入 ERP_08_工程進度表(執行一次)═══
// 來源=舊版全案件甘特圖的設計時程;忠泰湛改用最新排程(10/19開工~2027/01/04)。
// 已存在的「案件+工項」自動跳過,不會重複;跑完 App 甘特圖頁就會出現設計案長條。
function seedDesignGantt_0808() {
  var ROWS = [
    ['台北華府','泥作:防水/貼磁磚','2026/07/14','2026/08/04',''],
    ['台北華府','木作工程','2026/08/03','2026/08/10','8/10退場'],
    ['台北華府','立面施工圖繪製','2026/08/05','2026/08/20',''],
    ['鉅力高宇C-2F','3D發想・設計規劃','2026/07/24','2026/08/14','8/14材質挑選'],
    ['鉅力高宇C-2F','內部圖面與報價準備','2026/08/15','2026/08/31','8/31報價簽約'],
    ['鉅力高宇C-2F','預計工程期間','2026/09/15','2026/12/15','9/15吉日開工'],
    ['遠雄仰森A3-22','調整後3D＋迴路圖','2026/07/22','2026/07/31',''],
    ['遠雄仰森A3-22','工程估價與報價準備','2026/08/04','2026/08/26','8/26工程報價'],
    ['遠雄仰森A3-22','預計工程期間(暫)','2026/09/22','2026/12/31','待建商交屋確認'],
    ['忠泰湛B2-22F','兩版3D設計準備','2026/08/13','2026/08/29','8/29提兩版3D'],
    ['忠泰湛B2-22F','3D修改＋材質挑選','2026/09/01','2026/09/19',''],
    ['忠泰湛B2-22F','施工圖與工程報價','2026/09/22','2026/10/09',''],
    ['忠泰湛B2-22F','預計工程期間','2026/10/19','2027/01/04','10/19吉日開工'],
    ['合雄鳳璽','假日現場丈量','2026/08/15','2026/08/15','已約定8/15下午'],
    ['合雄鳳璽','3D發想・規劃','2026/08/17','2026/09/11','9/11第一次3D提案'],
    ['合雄鳳璽','二次3D＋材質定案','2026/09/14','2026/09/25',''],
    ['合雄鳳璽','工程估價與報價','2026/09/28','2026/10/12','10/12報價簽約'],
    ['合雄鳳璽','預計工程期間(暫)','2026/10/19','2027/03/05','業主10/15前騰空']
  ];
  var ss = SpreadsheetApp.openById(SS_ID);
  var sh = ss.getSheetByName('ERP_08_工程進度表');
  if (!sh) {
    sh = ss.insertSheet('ERP_08_工程進度表');
    sh.appendRow(['案件','工項','開始日','結束日','備註']);
    sh.setFrozenRows(1);
  }
  var existing = {};
  ...
  var added = 0;
  ROWS.forEach(function(r){
    if (existing[r[0] + '|' + r[1]]) return;
    sh.appendRow(r);
    added++;
  });
  Logger.log('✅ ERP_08 新增 ' + added + ' 列設計案時程(重複自動跳過)');
  try { sendTelegramSelfReminder('📐 甘特圖已補設計案時程 ' + added + ' 列'); } catch(e) {}
  return added;
}
