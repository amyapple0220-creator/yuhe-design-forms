...
function handleTelegramUpdate(update) {
  if (update.message) {
    ...
    if (/^(同步|\/sync)$/i.test(text)) {
      v3_sendTelegramTo(chatId,'🔄 檢查更新中…');
      try { var sr = syncFromGitHub(); if (sr === 'no_change') v3_sendTelegramTo(chatId,'✅ 已是最新版'); }
      catch(se) { v3_sendTelegramTo(chatId,'❌ 同步失敗:' + se.message); }
      return;
    }
